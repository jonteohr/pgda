...
public class SupporterRole extends ListenerAdapter {
	public void onGuildMemberRoleAdd(GuildMemberRoleAddEvent e) {
		if(e.getRoles().contains(e.getGuild().getRoleById("278439657358753792")) || e.getRoles().contains(e.getGuild().getRoleById("640863929039323146"))) {
			e.getGuild().addRoleToMember(e.getMember(), DefaultRoles.supporterDivider).complete();
			e.getGuild().addRoleToMember(e.getMember(), DefaultRoles.tierDivider).complete();
	...
	public void onGuildMemberRoleRemove(GuildMemberRoleRemoveEvent e) {
		if(e.getRoles().contains(e.getGuild().getRoleById("278439657358753792")) || e.getRoles().contains(e.getGuild().getRoleById("640863929039323146"))) {
			e.getGuild().removeRoleFromMember(e.getMember(), DefaultRoles.supporterDivider).complete();
			e.getGuild().removeRoleFromMember(e.getMember(), DefaultRoles.tierDivider).complete();
